...
/**
 * Cap a slug segment to a maximum length, trimming trailing dashes that
 * would otherwise be left dangling by a mid-word cut. Keeps slugs short
 * enough to avoid OS path-length limits (notably Windows MAX_PATH ~260).
 * @param {string} s
 * @param {number} [max=60]
 * @returns {string}
 */
function _capSlug(s, max) {
  const limit = typeof max === 'number' && max > 0 ? max : 60;
...
function createRun(workflow, name, opts) {
  ...
  if (name && String(name).trim().length > 0) {
    base = `${ds}-${_capSlug(slugifyName(name))}`;
  } else {
    base = `${ds}-${_capSlug(slugifyName(workflow))}-${timeStr(now)}`;
  }
