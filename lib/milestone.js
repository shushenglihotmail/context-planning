...
const { validatePhase } = require('./types');
...
function verifyMilestoneComplete(roadmapContent, phaseNums, root) {
  ...
  for (const num of phaseNums) {
    ...
    // v1.5: a phase that uses the pass-through shape (no plan checkboxes
    // in ROADMAP, one phase-level SUMMARY.md on disk) is a valid completed
    // shape. This is what `cp run milestone` produces in v1.5+. Pre-v1.5
    // phases keep the checklist rule (all checkboxes ticked + each plan
    // has its NN-MM-SUMMARY.md).
    const phaseSummaryPresent = !!(dir && fs.existsSync(path.join(dir, 'SUMMARY.md')));
...
function writeSummary(root, planId, summaryData, options = {}) {
  ...
  if (expectedCheck !== false) {
    ...
    // v0.9 P106 (Bug B): skip diff when expected is null (absent) or []
    // (explicitly empty). Only run when caller declared at least one file.
...
function _extractExpectedKeyFiles(phaseDir, planId) {
  ...
  // v0.9 P106 (Bug B): File validation is keyed off expected_files in
  // PLAN.md frontmatter, not body scraping. Missing frontmatter = no-op.
  // When present, expected_files defines the allowlist of files that
  ...
  // Empty list [] = explicitly no files expected; skip diff/warn (no fallback).
  if (fmObj && 'expected_files' in fmObj) {
    const raw = fmObj['expected_files'];
    if (Array.isArray(raw)) return _dedupStrings(raw);
    return null; // malformed scalar — treat as absent
  }

  // Legacy fallback: expected-key-files (hyphenated, complex per-plan or flat).
  if (!fmObj || !('expected-key-files' in fmObj)) {
    try { process.stderr.write(`cp: phase ${planId}: expected-key-files not found in PLAN.md frontmatter; skipping validation.\n`); } catch (_) {}
    return null;
  }
...
/**
 * Read every ROADMAP.md phase into the unified Phase shape.
 *
 * @param {string} roadmapMd
 * @returns {Array<object>}
 */
function readPhases(roadmapMd, opts = {}) {
  void opts;
  if (typeof roadmapMd !== 'string' || roadmapMd.trim().length === 0) return [];

  return roadmap.listPhases(roadmapMd).map((p) => {
    const block = roadmapMd.slice(p.blockStart, p.blockEnd);
    const plans = p.plans.map((plan) => ({
      id: plan.id,
      desc: plan.desc,
      done: !!plan.done,
    }));
    const meta = _extractRoadmapPhaseMeta(block);
    const phase = {
      id: String(p.num),
      depends_on: [],
      status: _deriveRoadmapPhaseStatus(plans),
      name: p.name,
      plans,
    };

    if (meta.workflow) phase.workflow = meta.workflow;
    if (meta.summary) phase.summary = meta.summary;
    if (meta['base-commit']) phase['base-commit'] = meta['base-commit'];

    const validation = validatePhase(phase);
    if (!validation.ok) {
      throw new Error(`invalid phase ${phase.id}: ${validation.errors.join('; ')}`);
    }
    return phase;
  });
}

function _deriveRoadmapPhaseStatus(plans) {
  if (!Array.isArray(plans) || plans.length === 0) return 'pending';
...
function _extractRoadmapPhaseMeta(block) {
  const fromFm = _extractRoadmapPhaseFrontmatter(block);
  const meta = {};
  const workflow = _cleanMetaString(fromFm.workflow) || _extractRoadmapAnnotation(block, 'Workflow') || _extractRoadmapAnnotation(block, 'workflow');
  const summary = _cleanMetaString(fromFm.summary) || _extractRoadmapAnnotation(block, 'Summary') || _extractRoadmapAnnotation(block, 'summary');
  const baseCommit =
    _cleanMetaString(fromFm['base-commit']) ||
    _cleanMetaString(fromFm.base_commit) ||
    _extractRoadmapAnnotation(block, 'Base Commit') ||
    _extractRoadmapAnnotation(block, 'base-commit') ||
    _extractRoadmapAnnotation(block, 'base_commit');

  if (workflow) meta.workflow = workflow;
  if (summary) meta.summary = summary;
  if (baseCommit) meta['base-commit'] = baseCommit;
  return meta;
}

function _extractRoadmapPhaseFrontmatter(block) {
  const fence = block.match(/(?:^|\r?\n)---[ \t]*\r?\n([\s\S]*?)\r?\n---[ \t]*(?:\r?\n|$)/);
  if (!fence) return {};
  try {
    return fm.parse(`---\n${fence[1]}\n---\n`).frontmatter || {};
  } catch (_) {
...
function _extractRoadmapAnnotation(block, label) {
  ...
  for (const re of patterns) {
    ...
    const cleaned = match ? _cleanMetaString(match[1]) : undefined;
...
/**
 * Create milestone-tier DESIGN.md and STATE.md files if absent.
 *
 * @param {string} milestoneSlug
 * @param {string} brief
 * @param {{root?: string}} [opts]
 * @returns {{designCreated: boolean, stateCreated: boolean}}
 */
function scaffoldTierFiles(milestoneSlug, brief, opts = {}) {
  const root = opts.root || '.planning';
  const dir = path.join(root, 'milestones', milestoneSlug);
  fs.mkdirSync(dir, { recursive: true });

  const milestoneName = _milestoneNameFromSlug(milestoneSlug);
  ...
  if (!fs.existsSync(designPath)) {
    fs.writeFileSync(designPath, _initialMilestoneDesign(milestoneSlug, milestoneName, brief, today), 'utf8');
    designCreated = true;
  }
  if (!fs.existsSync(statePath)) {
    fs.writeFileSync(statePath, _initialMilestoneState(milestoneSlug, milestoneName, today), 'utf8');
    stateCreated = true;
  }

  return { designCreated, stateCreated };
}

function _initialMilestoneDesign(slug, name, brief, today) {
  const body = String(brief || '').trim();
  return `---\n` +
    `milestone_slug: "${_yamlDoubleQuote(slug)}"\n` +
    `milestone: "${_yamlDoubleQuote(name)}"\n` +
    `status: draft\n` +
    `created: ${today}\n` +
    `updated: ${today}\n` +
    `deciders: []\n` +
...
function _initialMilestoneState(slug, name, today) {
  return `---\n` +
    `milestone_slug: "${_yamlDoubleQuote(slug)}"\n` +
    `milestone: "${_yamlDoubleQuote(name)}"\n` +
    `status: active\n` +
...
function _milestoneNameFromSlug(slug) {
  ...
  if (/^v\d+$/i.test(first)) {
    const nums = [];
    while (parts.length > 0 && /^\d+$/.test(parts[0])) nums.push(parts.shift());
    if (nums.length > 0) version = `${first}.${nums.join('.')}`;
  }
  const title = parts.map(_titleWord).join(' ');
  return title ? `${version} ${title}` : version;
}

function _titleWord(word) {
  if (/^[A-Z0-9]+$/.test(word)) return word;
  return word.charAt(0).toUpperCase() + word.slice(1);
}

function _yamlDoubleQuote(value) {
