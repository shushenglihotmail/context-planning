...
function writeSummary(root, planId, summaryData, options = {}) {
  ...
  // v0.8 P1: stamp end-commit when git is available. Forward-only; missing
  // field on pre-v0.8 SUMMARY.md parses cleanly (treated as undefined by
  // downstream audit, falling back to `confidence: low`).
  if (!('end-commit' in normalised)) {
    const endSha = git.headSha({ cwd: root });
    if (endSha) normalised['end-commit'] = endSha;
  }
