...
const { validatePreExpand } = require('./workflow-template-validate');
...
const CONFIG_FALLBACKS = Object.freeze({
  ...
  // v1.6 D3: extend with the remaining common role-keys so workflow authors
  // get the obvious Superpowers default for free instead of falling through
  // to "absent". See docs/superpowers/specs/2026-05-28-v1-6-workflow-contract-hardening-design.md.
...
function expandGroup(ctx) {
  ...
  for (let i = 0; i < def.phases.length; i++) {
    ...
    if (kind === 'template') {
      ...
      // v1.7: validate parameterization whitelist BEFORE substitution
      // so we get rule-specific error messages (field-not-parameterizable
      // vs. dotted-token-forbidden) instead of opaque substitution errors.
      validatePreExpand(innerRaw, {
        filePath: ctx.templateName,
