...
function expandGroup(ctx) {
  ...
  const def = loadWorkflowTemplate(ctx.templateName, { projectDir: ctx.projectDir });

  const mergedArgs = mergeArgs(def, ctx.callerArgs, ctx.templateName, ctx.callerLabel);
  ...
  for (let i = 0; i < def.phases.length; i++) {
    ...
    if (kind === 'template') {
      ...
      // bare or `phase:`-wrapped phase. Unwrap, substitute body.
      const innerRaw = (kind === 'bare') ? raw : raw.phase;
...
function mergeArgs(def, callerArgs, templateName, callerLabel) {
  const out = {};
