...
// v0.10.1: helper for "is digest already in MILESTONES.md" probe.
function escapeRegExpForMilestone(s) {
  return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}
...
function tickPlan(root, planId, options = {}) {
  ...
  if (!phaseDir) {
    throw new Error(`Phase ${phaseNum} dir not found under ${planning}/phases/. Run \`cp scaffold-phase ${phaseNum}\` (or \`/cp-autonomous\` to drive the whole milestone) first.`);
  }
...
// ---------- supersede + deviate (v0.8 P10) ----------

/**
 * Mark a plan as superseded. Replaces `- [ ]` / `- [x]` with `- [~]` in
 * ROADMAP.md AND the phase PLAN.md, and appends a "Superseded by" note to
 * the plan's section in PLAN.md (or to ## Notes if no plan section exists).
 *
 * Returns { actions, roadmapChanged, planChanged }.
 *
 * options:
 *   - by: string (new plan id) — required
 *   - reason: string (optional)
 *   - dryRun: boolean
 */
function supersedePlan(root, planId, options = {}) {
  const { by, reason = '', dryRun = false } = options;
  if (!by || typeof by !== 'string') {
    throw new Error('supersedePlan: --by <newPlanId> is required.');
  }
  if (!/^\d+(?:\.\d+)?-\d+$/.test(planId)) {
    throw new Error(`supersedePlan: invalid plan id "${planId}"`);
  }
  if (!/^\d+(?:\.\d+)?-\d+$/.test(by)) {
    throw new Error(`supersedePlan: invalid --by "${by}"`);
  }
  const { phaseNum } = parsePlanId(planId);
  const planning = paths.planningDir(root);
  const roadmapPath = path.join(planning, 'ROADMAP.md');
  if (!fs.existsSync(roadmapPath)) {
  ...
  const phaseDir = paths.findPhaseDir(phaseNum, root);
  if (!phaseDir) {
    throw new Error(`Phase ${phaseNum} dir not found.`);
  }
  const phasePlanPath = path.join(phaseDir, 'PLAN.md');
  ...
  const roadmapBefore = readFile(roadmapPath);
  const roadmapAfter = roadmap.setPlanSuperseded(roadmapBefore, planId);
  const roadmapChanged = roadmapBefore !== roadmapAfter;
  if (roadmapChanged) actions.push({ path: roadmapPath, before: roadmapBefore, after: roadmapAfter });
  ...
  if (fs.existsSync(phasePlanPath)) {
    const planBefore = readFile(phasePlanPath);
    let planAfter = roadmap.setPlanSuperseded(planBefore, planId);
    // Append note in ## Notes if no per-plan section. Always append a
    // dated line that captures the supersede event.
    const today = (options.today || new Date().toISOString().slice(0, 10));
    const noteLine = `- ${today}: plan ${planId} superseded by ${by}${reason ? ` — ${reason}` : ''}\n`;
    if (/^##\s+Notes\s*$/m.test(planAfter)) {
      planAfter = planAfter.replace(/(^##\s+Notes\s*$)/m, `$1\n\n${noteLine}`);
    } else {
      planAfter = planAfter.trimEnd() + `\n\n## Notes\n\n${noteLine}`;
    }
    planChanged = planBefore !== planAfter;
    if (planChanged) actions.push({ path: phasePlanPath, before: planBefore, after: planAfter });
  }
...
/**
 * Record a deliberate deviation from PLAN.md by appending a `## Deviation
 * YYYY-MM-DD` block to the phase PLAN.md (or in ## Notes if it exists).
 *
 * options:
 *   - summary: string — required, one-line summary of what diverged
 *   - reason:  string — optional, why we diverged
 *   - dryRun:  boolean
 *
 * Returns { actions, planChanged, phaseDir }.
 */
function recordDeviation(root, phaseNum, options = {}) {
  const { summary, reason = '', dryRun = false } = options;
  if (!summary || typeof summary !== 'string' || !summary.trim()) {
    throw new Error('recordDeviation: --summary "<text>" is required.');
  }
  const phaseDir = paths.findPhaseDir(String(phaseNum), root);
  if (!phaseDir) throw new Error(`Phase ${phaseNum} dir not found.`);
  const phasePlanPath = path.join(phaseDir, 'PLAN.md');
  if (!fs.existsSync(phasePlanPath)) {
    throw new Error(`Phase ${phaseNum} has no PLAN.md.`);
  }
  ...
  const before = readFile(phasePlanPath);
  // Append at end of file. Always a fresh ## Deviation N heading so multiple
  // deviations accumulate.
  const after = before.trimEnd() + '\n\n' + block + '\n';
  const actions = [];
  const planChanged = before !== after;
  if (planChanged) actions.push({ path: phasePlanPath, before, after });

  if (!dryRun) {
    for (const a of actions) writeFile(a.path, a.after);
  }
  return { actions, planChanged, phaseDir };
}
...
function scaffoldMilestone(root, name, options = {}) {
  ...
  // v0.9 P36 fix: insert the new milestone heading at the END of the
  // `## Phases` section (immediately before the next `## ` H2, or at EOF)
  // rather than at the TOP just after the section header. The TOP-insert
  // bug caused complete-milestone's collapse step (and findMilestoneInRoadmap)
  // to scoop ALL prior content into the new milestone, producing
  // "Phases 1-N" digests instead of just the milestone's own range.
  const afterPhasesHeader = before.slice(phasesIdx);
  const nextH2Rel = afterPhasesHeader.slice(2).search(/^##\s+/m);
  const sectionEnd = nextH2Rel === -1 ? before.length : phasesIdx + 2 + nextH2Rel;
  // Walk back past trailing whitespace so we don't leave gaps.
  let insertAt = sectionEnd;
  while (insertAt > phasesIdx && /\s/.test(before.charAt(insertAt - 1))) insertAt--;
...
function scaffoldPhase(root, phaseNum, options = {}) {
  const { dryRun = false, name, plans = 0, milestone: milestoneName, today: todayIso, force = false, continueFromPrior = false } = options;
  if (!name || typeof name !== 'string' || !name.trim()) {
  ...
  // bypasses (with stderr notice from CLI layer). v0.8 P10: --continue
  // also bypasses, but records a "Continues from phase N-1" note in the
  // new PLAN.md.
  let priorAudit = null;
  if (!force && !continueFromPrior) {
    let audit;
    try {
    ...
    // Capture the prior phase info for the Continues-from note.
    try {
      priorAudit = _priorPhaseAudit(root, numStr);
    } catch (_) { priorAudit = null; }
  }
  ...
  // v0.8 P10: --continue records a "Continues from phase N-1" note in the
  // new PLAN.md so the carry-over is auditable. Appended to ## Notes if
  // present, else appended at end of file.
  if (continueFromPrior) {
    const priorRef = priorAudit && priorAudit.priorPhase ? `phase ${priorAudit.priorPhase}` : 'prior phase (unaudited)';
    const missingLine = priorAudit && priorAudit.missingSummaries && priorAudit.missingSummaries.length > 0
      ? ` (missing summaries: ${priorAudit.missingSummaries.join(', ')})`
      : '';
    const note = `**Continues from**: ${priorRef}${missingLine}\n\nThis phase was scaffolded with \`--continue\`, carrying forward work\nthat had not been fully summarised.\n`;
    if (/^##\s+Notes\s*$/m.test(planFinal)) {
      planFinal = planFinal.replace(/(^##\s+Notes\s*$[\s\S]*?(?=\n##|$))/m, (m) => {
        return m.trimEnd() + '\n\n' + note;
      });
    } else {
      planFinal = planFinal.trimEnd() + `\n\n## Notes\n\n${note}`;
    }
  }

  // DESIGN.md (v0.7): persistent phase-tier design doc, populated by the
  // workflow's `plan` phase (v1.2+) or any provider plan/brainstorm skill.
...
function writeSummary(root, planId, summaryData, options = {}) {
  ...
  const phaseDir = paths.findPhaseDir(phaseNum, root);
  if (!phaseDir) {
    throw new Error(`Phase ${phaseNum} dir not found. Run \`cp scaffold-phase ${phaseNum}\` (or \`/cp-autonomous\`) first.`);
  }
...
function statusReport(root) {
  ...
  // v0.10.1: STATE.md fallback. When ROADMAP.md has no `### (In Progress)`
  // heading (e.g. the active milestone is already collapsed in <details>
  // because writing-plans wrapped it after the final phase), fall back to
  // STATE.md's `milestone:` field. findMilestoneInRoadmap will then pick
  // up the milestone via the <summary> path even if it's shipped.
  if (!currentMilestoneName && stateContent) {
    const stateMilestone = stateContent.match(/^\s*milestone\s*:\s*(.+?)\s*$/mi);
    if (stateMilestone) {
      const cand = stateMilestone[1].trim().replace(/^["']|["']$/g, '');
      if (cand && cand !== '-' && cand.toLowerCase() !== 'none' && cand.toLowerCase() !== 'idle') {
        currentMilestoneName = cand;
      }
    }
  }
...
function completeMilestone(root, options = {}) {
  ...
  // v0.10.1: already-shipped path. The milestone is already collapsed in
  // ROADMAP.md (either by a prior `cp complete-milestone` run, by the
  // Superpowers `writing-plans` skill's final-phase wrap, or by hand).
  // Make the verb idempotent: if MILESTONES.md already contains the
  // digest, exit clean as a no-op. Otherwise fall through and finish the
  // remaining bookkeeping (digest append + STATE reset + cleanup).
  if (found.status === 'shipped') {
    const digestAlready = fs.existsSync(milestonesPath)
      && new RegExp(`^##\\s+.*${escapeRegExpForMilestone(found.name)}`, 'mi')
        .test(readFile(milestonesPath));
    if (digestAlready) {
      return {
        ok: true,
        alreadyShipped: true,
        ...
        phases: found.phases,
        actions: [],
        message: `Milestone "${found.name}" already shipped (digest present in MILESTONES.md). No-op.`,
      };
    }
    // else: collapsed in ROADMAP but no digest yet — fall through. The
    // collapse step below will be a no-op (collapseMilestoneInRoadmap
    // returns changed:false when already collapsed), the digest append
    // and STATE reset will run normally.
    // v0.10.2: SKIP verify entirely. The collapsed state IS the source of
    // truth — whoever collapsed it (us, writing-plans, hand) declared
    // completeness. Re-verifying would block on phases that may have
    // been intentionally folded together or summary files written in a
    // non-cp location.
  }

  // Verify all phases done + summaries present.
  // v0.10.2: --force bypasses the verify gate (use after manual review).
  // v0.10.2: shipped (already-collapsed) milestones skip verify — the
  // collapsed state IS the source of truth.
  const { force = false } = options;
  let verify = null;
  if (found.status !== 'shipped' && !force) {
    verify = milestone.verifyMilestoneComplete(roadmapContent, found.phases, root);
    if (!verify.ok) {
      return { ok: false, reason: 'incomplete', milestone: found.name, phases: found.phases, verify, actions: [] };
    }
  }

  // v0.8 P7: audit gate. Run cp audit before any mutation.
  // - HIGH: always refuse.
  // - MEDIUM: refuse unless auditWarn.
  // - LOW: warn only (caller may render).
  // Escape: noAudit skips the gate entirely (emits override notice).
  // Fail-closed: runAudit throw → audit-error.
  const { noAudit = false, auditWarn = false } = options;
  let auditResult = null;
  if (noAudit) {
    auditResult = { skipped: true, reason: 'no-audit-flag' };
  } else {
    try {
      auditResult = audit.runAudit(root);
    } catch (err) {
      return {
        ok: false,
        reason: 'audit-error',
    ...
    const sum = auditResult.summary || { high: 0, medium: 0, low: 0 };
    const blockReason = sum.high > 0
      ? 'HIGH'
      : (sum.medium > 0 && !auditWarn ? 'MEDIUM' : null);
    if (blockReason) {
      return {
        ok: false,
        reason: 'audit-failed',
  ...
  if (dryRun) {
    return { ok: true, milestone: found.name, phases: found.phases, agg, verify, audit: auditResult, actions, dryRun: true };
  }
  ...
  // with the collapsed milestone. Failure is non-fatal — warn and continue.
  try { state.regenerate(root); } catch (err) {
    process.stderr.write(`cp: state regen failed after close-milestone: ${err.message}\n`);
  }
  ...
  return { ok: true, milestone: found.name, phases: found.phases, agg, verify, audit: auditResult, actions, commit };
}

// ---------- v0.8 P12 (Phase 28): auto-tick from post-commit hook ----------

/**
 * Decide whether to auto-tick `planId` based on a commit's changed files.
 *
 * Returns one of:
 *   { decision: 'tick', expected, matched }
 *   { decision: 'skip', reason: 'no-expected' }       — PLAN.md has no expected-key-files
 *   { decision: 'skip', reason: 'partial', missing }  — expected files NOT all touched
 *   { decision: 'skip', reason: 'no-files' }          — commit touched zero files
 *   { decision: 'skip', reason: 'no-phase-dir' }      — phaseDir not resolvable
 *
 * Pure-ish: takes file lists, NEVER touches disk except to read PLAN.md
 * frontmatter via milestone._extractExpectedKeyFiles. Caller decides
 * whether to invoke tickPlan() based on the decision.
 *
 * Path matching is POSIX-normalised on both sides (handles Windows
 * backslashes in `git show --name-only` output).
 */
function tryAutoTick(root, planId, commitFiles) {
  const { phaseNum, id } = parsePlanId(planId);
  const phaseDir = paths.findPhaseDir(phaseNum, root);
  if (!phaseDir) return { decision: 'skip', reason: 'no-phase-dir' };

  const files = (commitFiles || []).map((f) => String(f).replace(/\\/g, '/'));
  if (files.length === 0) return { decision: 'skip', reason: 'no-files' };

  const milestone = require('./milestone');
  let expected = null;
  try {
    expected = milestone._extractExpectedKeyFiles(phaseDir, id);
  } catch (_) {
    expected = null;
  }
  // expected may be array (flat) or null. For the object form,
  // _extractExpectedKeyFiles returns this plan's slot only.
  if (!expected || expected.length === 0) {
    return { decision: 'skip', reason: 'no-expected' };
  }

  const want = expected.map((f) => String(f).replace(/\\/g, '/'));
  const have = new Set(files);
  const missing = want.filter((w) => !have.has(w));
  if (missing.length > 0) {
    return { decision: 'skip', reason: 'partial', missing, expected: want };
  }
  return { decision: 'tick', expected: want, matched: want.length };
}
