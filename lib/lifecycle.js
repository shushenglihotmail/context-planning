...
function writeSummary(root, planId, summaryData, options = {}) {
  const { dryRun = false, body, overwrite = false, autoKeyFiles = true, checkFileExistence = true } = options;
  const { phaseNum, id } = parsePlanId(planId);
  ...
  return { path: summaryPath, action: dryRun ? 'dryrun' : 'written', fm: normalised, autoFill };
}
