...
function listPhases(content) {
  const phases = [];
  const headingRe = /^#{3,4}\s+Phase\s+([\d.]+):\s+(.+?)\s*(?:\(INSERTED\))?\s*$/gm;
  let match;
  const matches = [];
...
/**
 * v0.10.3: List phase numbers that appear inside a collapsed milestone
 * `<details><summary>✅ <name> (Phases X-Y) — SHIPPED ...</summary>` block.
 * Returns the union of:
 *   - integer ranges X..Y expanded ("Phases 14-16" → ["14","15","16"])
 *   - any `### Phase N:` headings still present inside the <details>
 * Used by callers (audit, status) that need to recognize phase numbers
 * implicitly declared via a collapsed milestone, even when writing-plans
 * has flattened the inner `### Phase N` headings away.
 */
function listCollapsedPhaseNums(content) {
  const out = new Set();
  const summaryRe = /<summary>\s*(?:[\u2705\u2611\u2713\u2714\u2728\s])*[^(<]+?\s*\(Phases?\s+([\d.]+)\s*[-\u2013\u2014]\s*([\d.]+)\)\s*[-\u2013\u2014]\s*SHIPPED[^<]*<\/summary>/gi;
  let m;
  while ((m = summaryRe.exec(content)) !== null) {
    const detailsStart = m.index;
    const detailsClose = content.indexOf('</details>', detailsStart);
    const innerEnd = detailsClose === -1 ? content.length : detailsClose;
    const inner = content.slice(detailsStart, innerEnd);
    // Inner explicit headings (if writing-plans preserved them).
    const phRe = /^#{3,4}\s+Phase\s+([\d.]+):/gim;
    let pm;
    while ((pm = phRe.exec(inner)) !== null) out.add(pm[1]);
