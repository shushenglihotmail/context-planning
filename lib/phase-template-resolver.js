...
function resolvePhaseTemplateRef(phaseEntry, opts) {
  ...
  // v1.4: `description:` is a caller-supplied field (Decision #2) — it
  // describes the specific use of the template, not the template itself.
  // The template body should not carry description; if the caller provides
  // one, splice it in so validate() sees it.
  if (Object.prototype.hasOwnProperty.call(phaseEntry, 'description')) {
    out.description = phaseEntry.description;
  }
