...
/**
 * cp autonomous — v0.10 P36 orchestrator.
 *
 * Walks pending phases of the active milestone, delegating per-phase
 * planning + execution to caller-supplied hooks (`opts.planPhase` and
 * `opts.executePhase`). Smart-gated on test failure, audit HIGH, and
 * executor deviation; stops cleanly by writing `.planning/.continue-here.md`.
 *
 * Pure orchestrator: no agent reasoning, no plan/execute logic itself.
 * The skill layer in phase 37 supplies the real `planPhase` /
 * `executePhase` callbacks that delegate to `/cp-plan-phase` and
 * `/cp-execute-phase` via the harness. In unit tests, callbacks are
 * stubbed.
 *
 * See: docs/superpowers/specs/2026-05-21-v0-10-cp-autonomous-design.md
 */
...
function writeContinueHere(root, ctx) {
  ...
  const lines = [
    ...
    `  the next pending plan in phase ${ctx.failedPhase}`,
    '',
 ...
 *   planPhase: async fn(phaseNum)        delegated planner; required when
 *                                        any phase has a stub PLAN.md
 *   executePhase: async fn(phaseNum, planId) delegated executor; required
 *                                        unless dryRun
 ...
 *
 * Returns:
 *   On success: { ok: true, scope, milestone, startPhase, phasesProcessed, stopped: false }
 *   On stop:    { ok: false, ..., stopped: true, stopReason, failedPhase, failedPlan, details, continueHere }
 *   On dry-run: { ok: true, dryRun: true, scope, phasesWouldRun, totalPlans }
 */
async function runAutonomous(root, opts = {}) {
  ...
  // Step 3: execute loop
  if (typeof opts.executePhase !== 'function') {
    return {
      ok: false,
      reason: 'missing-executor',
      message: 'opts.executePhase callback is required (skill layer supplies it).',
    };
  }
  ...
  for (const phaseNum of phasesInfo.phases) {
    ...
    // Step 3a: ensure PLAN.md is non-stub
    const phaseDir = paths.findPhaseDir(phaseNum, root);
    const planPath = phaseDir ? path.join(phaseDir, 'PLAN.md') : null;
    const planIsStub = !planPath || !fs.existsSync(planPath) ||
      _isStubPlan(fs.readFileSync(planPath, 'utf8'));
    if (planIsStub) {
      if (typeof opts.planPhase !== 'function') {
        return {
          ok: false,
          scope: scope.kind,
          milestone: phasesInfo.milestoneName,
          ...
          phasesProcessed,
          stopped: true,
          stopReason: 'plan-stub',
          failedPhase: phaseNum,
          message: `Phase ${phaseNum} PLAN.md is stub and no planPhase callback supplied.`,
        };
      }
      try {
        await opts.planPhase(phaseNum);
      } catch (e) {
        const continueHere = writeContinueHere(root, {
          failedPhase: phaseNum, reason: 'plan-failed', details: e.message,
        });
        return {
          ok: false, scope: scope.kind, milestone: phasesInfo.milestoneName,
          startPhase: start.phase, phasesProcessed, stopped: true,
          stopReason: 'plan-failed', failedPhase: phaseNum,
          details: e.message, continueHere,
        };
      }
    }

    // Step 3b: walk pending plans
    const phaseStart = Date.now();
    let plansProcessed = 0;
    for (const plan of ph.plans) {
      if (plan.done) continue;
      try {
        await opts.executePhase(phaseNum, plan.id);
      } catch (e) {
        const continueHere = writeContinueHere(root, {
          failedPhase: phaseNum, failedPlan: plan.id,
          reason: 'deviation', details: e.message,
        });
        return {
          ok: false, scope: scope.kind, milestone: phasesInfo.milestoneName,
          startPhase: start.phase, phasesProcessed, stopped: true,
          stopReason: 'deviation', failedPhase: phaseNum, failedPlan: plan.id,
          details: e.message, continueHere,
        };
      }
      plansProcessed += 1;
      ...
      if (!opts.skipTests) {
        ...
        if (!tr.ok) {
          const continueHere = writeContinueHere(root, {
            failedPhase: phaseNum, failedPlan: plan.id,
            reason: 'test-failure', details: tr.output,
          });
          return {
            ok: false, scope: scope.kind, milestone: phasesInfo.milestoneName,
            startPhase: start.phase, phasesProcessed, stopped: true,
            stopReason: 'test-failure', failedPhase: phaseNum, failedPlan: plan.id,
      ...
      if (!opts.skipAudit) {
        ...
        if (!ar.ok) {
          ...
          const continueHere = writeContinueHere(root, {
            failedPhase: phaseNum, failedPlan: plan.id,
            reason: 'audit-high', details,
          });
          return {
            ok: false, scope: scope.kind, milestone: phasesInfo.milestoneName,
            startPhase: start.phase, phasesProcessed, stopped: true,
            stopReason: 'audit-high', failedPhase: phaseNum, failedPlan: plan.id,
  ...
  if (scope.kind === 'milestone' && typeof opts.runCompleteMilestone === 'function') {
    try {
      ...
      return {
        ok: false, scope: scope.kind, milestone: phasesInfo.milestoneName,
        startPhase: start.phase, phasesProcessed, stopped: true,
        stopReason: 'close-failed', details: e.message,
      };
    }
  }
...
function _isStubPlan(body) {
  if (!body) return true;
  if (/\{Describe what this phase delivers in 1-2 sentences\.\}/.test(body)) return true;
  if (/\{brief description\}/.test(body)) return true;
  return false;
}
