...
const ALLOWED_BINDS = ['milestone', 'phase', 'custom'];
const WHITE = 0;
const GRAY = 1;
const BLACK = 2;
...
function validate(template) {
  ...
  if (
    ...
    errors.push('meta.binds_to must be one of: milestone, phase, custom');
  }
  ...
  for (let i = 0; i < safeTemplate.phases.length; i++) {
    ...
    phaseInfos.push({ i, id, validId, depends, dependsArray, depsAllStrings });
  }
