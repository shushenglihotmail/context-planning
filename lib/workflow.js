...
function computeWavesWithFanout(template, parentOutputs) {
  ...
  const merged = expanded.map(function (row) {
    const tpl = tplById.get(row.templateId) || {};
  ...
  for (const m of merged) {
    for (const d of depsFor(m)) {
      if (!rowById.has(d)) continue;
      indegree.set(m.id, indegree.get(m.id) + 1);
      dependents.get(d).push(m.id);
    }
  }
  ...
  while (remaining.size > 0) {
    ...
    if (waveIds.length === 0) {
      throw new Error('Cycle detected while computing fan-out waves');
    }
    waves.push(waveIds.map(function (id) { return rowById.get(id); }));
    for (const id of waveIds) {
      remaining.delete(id);
      for (const next of dependents.get(id)) {
        indegree.set(next, indegree.get(next) - 1);
