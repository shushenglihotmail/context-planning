...
function loadTemplate(nameOrPath, opts) {
  ...
  if (Array.isArray(phases)) {
    ...
    for (let i = 0; i < phases.length; i++) {
      ...
      if (ph._wrapperKind === 'template') continue; // workflow-template inclusion → Phase 55
...
function validate(template) {
  ...
  for (let i = 0; i < safeTemplate.phases.length; i++) {
    ...
    if (isTemplateEntry) {
      // v1.3: workflow-template inclusion. Resolution lives in Phase 55.
      // For now, surface a guard error so callers don't silently treat the
      // entry as a real phase. Field-rules enforcement (53-03) is applied
      // below; the entry is excluded from DAG analysis.
      templatesPresent = true;
      errors.push(
        `phases[${i}] (id: '${id}'): template entry resolution not yet implemented (Phase 55)`
      );
    ...
    if (isPhaseTemplateRef) {
      templatesPresent = true;
      errors.push(
        `phases[${i}] (id: '${id}'): phase-template reference resolution not yet implemented (Phase 54)`
      );
