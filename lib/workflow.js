...
const {
  validatePreExpand,
  validatePostExpand,
} = require('./workflow-template-validate');
...
function loadTemplate(nameOrPath, opts) {
  ...
  // v1.7: pre-expand parameterization-whitelist validation on every
  // top-level workflow file, BEFORE any substitution. Catches forbidden
  // tokens in id/after/depends_on/etc with precise error messages.
  // (Workflow-template inclusions get the same check inside expandGroup.)
  const validatorTemplateName = isPathLike(nameOrPath)
  ...
  if (Array.isArray(phases)) {
    for (let i = 0; i < phases.length; i++) {
      ...
      try {
        validatePreExpand(ph, {
          filePath: validatorTemplateName,
          skipFields: ['_wrapperKind'],
        });
      } catch (err) {
        resolverErrors.push(
          `phases[${i}]: ${err && err.message ? err.message : String(err)}`
        );
      }
    }
  }
  ...
  const supervisorTokenNames = new Set();
  if (topLevelParams.length > 0) {
    ...
    for (const p of topLevelParams) {
      ...
      if (Object.prototype.hasOwnProperty.call(p, 'default')) {
        ...
        // No default → declared "supervisor-supplied" token. Allowed:
        // the param's name is added to the post-expand allow-list so
        // leftover {{name}} tokens (substituted at run-time by the
        // supervisor) don't trip the validator. Pre-v1.7 this was an
        // error; v1.7 makes it the canonical way to declare run-time
        // injection points.
        supervisorTokenNames.add(p.name);
      }
    }
  ...
  if (Array.isArray(phases)) {
    ...
    // v1.7: post-expand parameterization-whitelist validation. After
    // every substitution pass (top-level params + workflow-template
    // expansion), no `{{...}}` token may remain. Catches the
    // `allowUndeclared: true` escape hatch at L116 and any dotted token
    // (e.g. {{item.id}}) that bypassed pre-expand because it appeared
    // in a workflow that did not declare params.
    for (let i = 0; i < phases.length; i++) {
      const ph = phases[i];
      if (!ph || typeof ph !== 'object' || Array.isArray(ph)) continue;
      try {
        validatePostExpand(ph, {
          filePath: validatorTemplateName,
          skipFields: ['_wrapperKind'],
          allowedTokenNames: supervisorTokenNames,
        });
      } catch (err) {
        resolverErrors.push(
          `phases[${i}]: ${err && err.message ? err.message : String(err)}`
...
function validate(template) {
  ...
  validateV12Schema(phaseInfos.filter(function (info) { return !info.isTemplateEntry; }), warnings, errors, meta);
  ...
  if (canAnalyzeGraph) {
    ...
    const topInfos = phaseInfos.filter(function (info) { return !info.parentSet; });
    // Remap any top-level depends_on that references a child to the child's parent.
    const childToParentInfo = new Map();
    for (const info of phaseInfos) {
      if (info.parentSet && typeof info.parent === 'string') {
        childToParentInfo.set(info.id, info.parent);
      }
    }
    const remappedTopInfos = topInfos.map(function (info) {
      const seen = new Set();
      const newDeps = [];
      for (const d of info.depends) {
        const r = childToParentInfo.has(d) ? childToParentInfo.get(d) : d;
        if (!seen.has(r) && r !== info.id) { seen.add(r); newDeps.push(r); }
      }
      return Object.assign({}, info, { depends: newDeps });
    });
    const topo = topoSortIds(remappedTopInfos);
    if (topo.length === remappedTopInfos.length) {
      const fileOrder = remappedTopInfos.map(function (info) { return info.id; });
...
function validateV12Schema(phaseInfos, warnings, errors, meta) {
  const isSupervised = !!(meta && meta.supervised === true);
  ...
  for (const info of phaseInfos) {
    ...
    if (Object.prototype.hasOwnProperty.call(phase, 'materialize')) {
      ...
      if (m !== 'inline' && m !== 'roadmap-phases') {
        ...
        // v1.8 P96: roadmap-phases parents must declare children (an
        // execution layer) unless the workflow is supervised, in which
        // case the supervisor dispatches children dynamically at runtime.
        if (m === 'roadmap-phases' && !isSupervised) {
          errors.push(
...
function computeWaves(template) {
  ...
  // Child phases (those with `parent:` set) are materialized at runtime by
  // lib/fanout.js#expandPhases when the parent's structured-list output is
  // consumed. They are not part of the top-level wave plan and must be
  // hidden from `cp workflow inspect` and the wave-driving runtime.
  // Build a child-id -> parent-id map so any top-level `depends_on` that
  // references a child (e.g. `review.depends_on: [write-child]`) is
  // remapped to the child's parent (the parent's completion implies all
  // its fanned-out children have completed).
  const childToParent = new Map();
  for (const p of template.phases) {
    if (p && Object.prototype.hasOwnProperty.call(p, 'parent') && typeof p.parent === 'string') {
      childToParent.set(p.id, p.parent);
    }
  }
  const phases = template.phases.filter(function (p) {
  ...
  for (const phase of phases) {
    const rawDeps = Array.isArray(phase.depends_on) ? phase.depends_on : [];
    const deps = [];
    const seen = new Set();
    for (const d of rawDeps) {
      const remapped = childToParent.has(d) ? childToParent.get(d) : d;
      if (!seen.has(remapped) && remapped !== phase.id) {
        seen.add(remapped);
        deps.push(remapped);
      }
    }
    depsById.set(phase.id, deps);
  }

  const idWaves = kahnWaves(ids, function (id) { return depsById.get(id) || []; });
  return idWaves.map(function (waveIds) {
    return waveIds.map(function (id) { return idToPhase.get(id); });
  });
}
...
function topoSortIds(phaseInfos) {
  ...
  // Permissive: validate() distinguishes acyclic vs cyclic input by comparing
  // returned length to input length. Preserve that by swallowing cycle errors
  // and returning an empty order so the caller's length-check fails cleanly.
  let waves;
  try {
    waves = kahnWaves(ids, function (id) { return depsById.get(id) || []; });
  } catch (_) {
    return [];
  }
  const order = [];
  for (const wave of waves) {
    for (const id of wave) order.push(id);
  }
  return order;
}
...
/**
 * Like computeWaves, but materialises fan-out children when their parent's
 * structured-list output is available in `parentOutputs`.
 *
 * Semantics:
 *   - When `parentOutputs` is empty/missing, returns the same shape as
 *     `computeWaves(template)`. (Child phases with `parent:` are filtered out;
 *     top-level `depends_on` referencing children is remapped to the parent.)
 *   - When `parentOutputs[parentId]` is present, the parent's child template
 *     phases are expanded per-item via `fanout.expandPhases` and injected into
 *     the wave plan after the parent's wave. Cross-item ordering follows the
 *     parent output's `optimizable` flag (array mode is the safe default).
 *   - Subtree-wait is enforced: any top-level phase whose `depends_on` /
 *     `after` references an expanded parent waits for all of that parent's
 *     expanded children (not just the parent itself).
 *   - Expanded child rows carry a synthetic `id` of the form
 *     `<templateId>::<itemId>` plus `templateId`, `parent`, `item`, and
 *     `itemIndex` fields so callers (e.g. runtime.formatInstruction) can
 *     reconstruct the per-item context.
 *
 * @param {object} template Loaded workflow template (already auto-injected-finalize).
 * @param {Record<string, {optimizable?:boolean, items:Array<object>}|Array<object>>} parentOutputs
 *        Map of parent phase id → parsed structured-list output. May be empty.
 * @returns {Array<Array<object>>} Waves of phase objects (template body merged with row info).
 */
function computeWavesWithFanout(template, parentOutputs) {
  const outputs = parentOutputs && typeof parentOutputs === 'object' ? parentOutputs : {};
  if (Object.keys(outputs).length === 0) {
    return computeWaves(template);
  }

  // Lazily require to avoid a circular import at load time.
  const fanout = require('./fanout');

  // Validate first — mirrors computeWaves's behaviour.
  const result = validate(template);
  if (result.errors.length > 0) {
    throw new Error(`Invalid workflow template: ${result.errors.join('; ')}`);
  }

  const unified = phasesFromTemplate(template);
  const expanded = fanout.expandPhases(unified, outputs);

  // Template-body lookup by original id (== row.templateId).
  ...
  // Build merged phase objects: template body + row identity/runtime context.
  const merged = expanded.map(function (row) {
    const tplBody = tplById.get(row.templateId) || {};
    const m = Object.assign({}, tplBody);
    m.id = row.id;
    m.templateId = row.templateId;
    if (Object.prototype.hasOwnProperty.call(row, 'parent')) m.parent = row.parent;
    if (Object.prototype.hasOwnProperty.call(row, 'item')) m.item = row.item;
    if (Object.prototype.hasOwnProperty.call(row, 'itemIndex')) m.itemIndex = row.itemIndex;
    Object.defineProperty(m, '_expandedAfter', {
      value: Array.isArray(row.after) ? row.after.slice() : [],
  ...
  // parentId -> [expanded child ids] for subtree-wait remap.
  const parentToChildren = new Map();
  for (const m of merged) {
    if (m._source === 'expanded' && typeof m.parent === 'string') {
      if (!parentToChildren.has(m.parent)) parentToChildren.set(m.parent, []);
      parentToChildren.get(m.parent).push(m.id);
    }
  }

  const rowById = new Map();
  for (const m of merged) rowById.set(m.id, m);

  function depsFor(m) {
    const base = (m._expandedAfter || []).slice();
    // Expanded children depend on their parent phase (the parent runs first,
    // produces the structured-list, then children execute).
    if (m._source === 'expanded' && typeof m.parent === 'string' && !base.includes(m.parent)) {
      base.push(m.parent);
    }
    const out = [];
    const seen = new Set();
    for (const d of base) {
      // Subtree-wait: if d is a parent we expanded, callers waiting on it
      // must wait on all its expanded children. Exception: if d IS this
      // row's own parent, that's the child→parent edge we just added; do
      // not remap or we'd create a self-cycle.
      if (parentToChildren.has(d) && parentToChildren.get(d).length > 0 && d !== m.parent) {
        if (!seen.has(d)) { seen.add(d); out.push(d); }
        for (const cid of parentToChildren.get(d)) {
          if (cid !== m.id && !seen.has(cid)) { seen.add(cid); out.push(cid); }
        }
      } else if (d !== m.id && !seen.has(d)) {
        seen.add(d);
        out.push(d);
      }
    }
  ...
  const ids = merged.map(function (m) { return m.id; });
  const rowIds = new Set(ids);
  const depsByRowId = new Map();
  for (const m of merged) {
    const deps = depsFor(m).filter(function (d) { return rowIds.has(d); });
    depsByRowId.set(m.id, deps);
  }

  const idWaves = kahnWaves(ids, function (id) { return depsByRowId.get(id) || []; });
  return idWaves.map(function (waveIds) {
    return waveIds.map(function (id) { return rowById.get(id); });
  });
}

/**
 * Kahn-style topological sort that returns nodes grouped into waves.
 *
 * Each wave contains every node whose dependencies (per `depsFor`) are already
 * satisfied by earlier waves. Within a wave, the original declaration order
 * from `nodeIds` is preserved. Throws on cycles, naming the stuck nodes.
 *
 * @private
 * @param {string[]} nodeIds Declaration-ordered list of node identifiers.
 * @param {(id: string) => string[]} depsFor Returns the dependency ids for a node.
 *   Unknown ids in the returned list are ignored (callers must pre-filter if
 *   strictness is needed).
 * @returns {string[][]} Array of waves; each wave is an array of node ids.
 * @throws {Error} `Cycle detected; stuck nodes: <ids>` when a cycle blocks progress.
 */
function kahnWaves(nodeIds, depsFor) {
  ...
  while (remaining.size > 0) {
    ...
    if (waveIds.length === 0) {
      throw new Error(`Cycle detected; stuck nodes: ${[...remaining].join(', ')}`);
    }
