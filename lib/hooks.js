...
const HOOKS = ['pre-commit', 'post-commit']; // phase 27 + 28
...
/**
 * Return { subject, files } for the most recent commit in `cwd`, or
 * `null` if not in a git repo / no commits. `files` is an array of
 * repo-root-relative POSIX paths.
 */
function lastCommitInfo(cwd = process.cwd()) {
  try {
    const subject = execSync('git log -1 --format=%s', {
      ...
      .toString()
      .trim();
    const filesRaw = execSync('git show -1 --name-only --format=%n HEAD', {
      cwd,
      stdio: ['ignore', 'pipe', 'ignore'],
    })
      .toString();
