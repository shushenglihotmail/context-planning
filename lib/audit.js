...
function checkStateStale(root, ctx) {
  const out = [];
  let derived;
  try { derived = state.deriveState(root); } catch (_) { return out; }
  if (!derived) return out;
  const statePath = path.join(paths.planningDir(root), 'STATE.md');
  const raw = readSafe(statePath);
  if (!raw) return out;
  const split = state._splitState ? state._splitState(raw) : null;
  const rendered = state._renderDerivedBlock ? state._renderDerivedBlock(derived) : null;
  if (!split || !rendered) return out;
  if (split.derivedBlock.trim() !== rendered.trim()) {
    out.push(mkFinding({
      id: 'state-stale',
      severity: 'LOW',
