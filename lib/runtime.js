...
const { buildParentPrompt, parseParentOutput, enforceChildCount } = require('./runtime-fanout');

// ---------- module-level constants ----------

/**
 * Separator between child template id and item id when forming an expanded
 * child phase id (e.g. `child-plan::item-1`).
 * @private
 */
const FANOUT_CHILD_ID_SEP = '::';

/**
 * Value of `phase.materialize` that triggers per-item ROADMAP fan-out.
 * @private
 */
const MATERIALIZE_ROADMAP_PHASES = 'roadmap-phases';
...
/**
 * Materialise a `materialize: roadmap-phases` parent phase's structured-list
 * output into per-item ROADMAP phase directories.
 *
 * Parses the parent's SUMMARY text, enforces min/max child count, scaffolds one
 * ROADMAP phase per item (writing a contract-shaped PLAN.md from
 * item.title/item.summary), and computes patches the caller must merge into
 * its mutable copies of `parent_outputs` and `phaseNumByPhaseId` before
 * persisting state.
 *
 * Side effects (filesystem only — does NOT mutate `state`):
 *   - Scaffolds N new `.planning/phases/<n>-<id>/` directories via lifecycle.
 *   - Overwrites each new phase's PLAN.md with `# title\n\nsummary\n`.
 *
 * @private
 * @param {object} currentPhase The just-completed parent phase row from the
 *   wave; must have `materialize === 'roadmap-phases'` and `parent == null`.
 * @param {string} summaryText The parent's SUMMARY.md body containing a fenced
 *   JSON block matching the fan-out contract.
 * @param {object} state Current milestone RUN.yaml state (read-only here).
 * @param {string} projectDir Absolute path to the project root.
 * @param {{ tpl: object }} opts Carries the loaded template so child template
 *   ids (rows whose `parent === currentPhase.id`) can be mapped to phase nums.
 * @returns {{
 *   parent_outputs_patch: object,
 *   phaseNumByPhaseId_patch: object
 * }} Patches to be merged into the caller's parentOutputs and
 *   phaseNumByPhaseId maps respectively.
 * @throws {Error} If parseParentOutput rejects the contract, enforceChildCount
 *   detects min/max violation, or scaffoldPhase fails for any item.
 */
function materializeRoadmapPhases(currentPhase, summaryText, state, projectDir, opts) {
  const o = opts || {};
  const tpl = o.tpl || {};
  const phaseId = currentPhase.id;
  const parsed = parseParentOutput(summaryText);
  enforceChildCount(currentPhase, parsed);

  const milestoneName = state.milestoneName || state.slug;
  // Identify child template ids so per-item phaseNum lookups can resolve
  // expanded child phase ids (e.g. 'child-plan::item-1') back to one dir.
  const childTemplateIds = (tpl.phases || [])
    .filter(function (p) { return p && p.parent === phaseId; })
    .map(function (p) { return p.id; });

  const phaseNumPatch = {};
  let n = nextPhaseNum(projectDir);
  for (const item of parsed.items) {
    const scaffold = lifecycle.scaffoldPhase(projectDir, n, {
      name: item.id,
      force: false,
    ...
    if (!scaffold.ok) {
      throw new Error(
        `materialize:roadmap-phases: scaffoldPhase failed for item '${item.id}' (phase ${n}): ${scaffold.reason}`
      );
    }

    // Write per-item PLAN.md body (title + summary). The phase template's
    // PLAN.md already exists from the scaffold; we replace it with a
    // contract-shaped stub the child-plan phase can expand.
    const phaseDirPath = paths.findPhaseDir(String(n), projectDir);
    if (phaseDirPath) {
      ...
      const summary = typeof item.summary === 'string' && item.summary.trim()
        ...
        : '(no summary provided)';
      const body = `# ${title}\n\n${summary}\n`;
      lifecycle.writeFile(path.join(phaseDirPath, 'PLAN.md'), body);
    }

    phaseNumPatch[item.id] = n;
    for (const ctid of childTemplateIds) {
      phaseNumPatch[`${ctid}${FANOUT_CHILD_ID_SEP}${item.id}`] = n;
    }
    n++;
  }

  return {
    parent_outputs_patch: { [phaseId]: parsed },
    phaseNumByPhaseId_patch: phaseNumPatch,
  };
}
 ...
 *   - "routing-key"      — skill matched a routing key in the active provider's
 *                          skills map; name is the resolved provider skill name
 *   - "pinned"           — skill is already a literal provider skill name
 *   - "pass-through"     — unknown skill, but exists in active provider catalog
 *                          (verified via skillExists) OR no plugin_shape provider
 *                          to verify against
 *   - "fuzzy-match"      — sigil hint matched to `name` via findFuzzyMatch.
 *                          `candidates` is the scored list, `fuzzy: true`.
 *   - "fuzzy-miss"       — sigil hint had no candidate >= threshold; fell through
 *   - "exact-missing"    — non-sigil literal does not exist in active provider
 *                          catalog; fell through
 *   - "manual-prompt"    — chain fallback to manual provider's prompts[role];
 *                          `fallbackPrompt` is the inline body
 *   - "subagent-dispatch" — chain last-resort directive (no inline prompt body)
 *
 * @param {string|null|undefined} phaseSkill
 * @param {object} [opts] - { projectDir, warningsOut, phaseRole }
 * @returns {object}
 */
function resolvePhaseSkill(phaseSkill, opts) {
  ...
  const phaseRole = opts && opts.phaseRole;
  let cfg;
  try {
  ...
  // v1.10: parse sigil. Author-side syntax errors throw loudly.
  let parsed;
  try {
    parsed = provider.parseSkillName(phaseSkill);
  } catch (e) {
    // Malformed sigil — author bug. Surface as a warning and pass-through raw.
    if (opts && Array.isArray(opts.warningsOut)) {
      opts.warningsOut.push(`Malformed skill name "${phaseSkill}": ${e.message}`);
    }
    return { name: phaseSkill, source: 'pass-through' };
  }

  // v1.10: sigil-fuzzy path
  if (parsed.fuzzy) {
    const catalog = provider.listProviderSkills(activeName, cfg, projectDir);
    const match = provider.findFuzzyMatch(parsed.name, catalog);
    if (match.chosen) {
      return {
        name: match.chosen,
        source: 'fuzzy-match',
    ...
    // Fuzzy miss → chain fallback (manual prompt → subagent-dispatch)
    return _chainFallback(parsed.name, phaseRole, projectDir, 'fuzzy-miss');
  }

  // Routing key under the active provider?
  if (Object.prototype.hasOwnProperty.call(activeSkills, parsed.name)) {
    const resolved = provider.resolveSkill(parsed.name, projectDir);
    const name = (resolved && resolved.skill) || parsed.name;
  ...
  for (const provName of Object.keys(providers)) {
    ...
    for (const k of Object.keys(skills)) {
      if (skills[k] === parsed.name) {
        return { name: parsed.name, source: 'pinned' };
      }
    }
  }

  // v1.10: verify exact-name skill exists in active provider catalog before
  // emitting pass-through. If not present, chain-fall-back.
  if (!provider.skillExists(activeName, parsed.name, cfg, projectDir)) {
    // Only chain-fall-back if the active provider has a real on-disk catalog
    // (i.e., listProviderSkills returns non-empty). Otherwise pass-through
    // preserves legacy behavior for harnesses without a probe-able catalog.
    const catalog = provider.listProviderSkills(activeName, cfg, projectDir);
    if (catalog.length > 0) {
      const fb = _chainFallback(parsed.name, phaseRole, projectDir, 'exact-missing');
      fb.missingFrom = activeName;
      return fb;
    }
  }
  ...
  if (opts && Array.isArray(opts.warningsOut)) {
    opts.warningsOut.push(
      `Unknown skill "${parsed.name}" — not a routing key for provider "${activeName}" ` +
      `and not a registered provider skill name. Passing through as-is.`
    );
  }
  return { name: parsed.name, source: 'pass-through' };
}

/** v1.10 chain fallback: manual-prompt → subagent-dispatch */
function _chainFallback(name, phaseRole, projectDir, missSource) {
  if (phaseRole) {
    const prompt = provider.resolvePromptForRole(phaseRole, projectDir);
    if (prompt) {
      return {
        name,
  ...
  return { name, source: 'subagent-dispatch', missSource, role: phaseRole || null };
}

/**
 * v1.10: format the per-phase `[resolution] ...` diagnostic line.
 * Returns null for clean exact matches (no line emitted).
 */
function _formatResolutionLine(r, rawSkill) {
  if (!r || r.source === 'absent') return null;
  const hint = (typeof rawSkill === 'string') ? rawSkill : '';
  switch (r.source) {
    case 'fuzzy-match': {
      const top = (r.candidates || []).slice(0, 3).map((c) => c.name).join(', ');
      return `  [resolution] fuzzy: ${hint} → ${r.name}  (candidates: ${top || '—'})`;
    }
    case 'exact-missing':
      return `  [resolution] exact: ${r.name} not in ${r.missingFrom || 'active'} catalog; falling back`;
    case 'manual-prompt': {
      const why = r.missSource === 'fuzzy-miss'
        ? `fuzzy "${hint}" had no match`
        : `${r.name} not in active catalog`;
      return `  [resolution] fallback: ${why}; manual provider prompt for role "${r.role || '(none)'}" inlined below`;
    }
    case 'subagent-dispatch': {
      const why = r.missSource === 'fuzzy-miss'
        ? `fuzzy "${hint}" had no match`
        : `${r.name} not in active catalog`;
      return `  [resolution] fallback: ${why}; dispatch a subagent with role "${r.role || '(none)'}" using the phase prompt above`;
    }
    default:
      return null;
  }
...
function formatInstruction(template, wave, waveIndex, opts) {
  ...
  for (const phase of wave) {
    ...
    const basePrompt = (phase.prompt != null) ? String(phase.prompt) : '';
    let promptText = (phase.materialize === 'roadmap-phases' && phase.parent == null)
      ? buildParentPrompt(phase, basePrompt)
      : basePrompt;

    // v1.10: when chain falls back to manual-prompt, inline the role prompt
    // into the phase prompt body so the agent has it right there.
    if (resolvedSkill.source === 'manual-prompt' && resolvedSkill.fallbackPrompt) {
      const sep = promptText.endsWith('\n') ? '\n' : '\n\n';
      promptText = `${promptText}${sep}[manual-prompt]\n${resolvedSkill.fallbackPrompt}\n[/manual-prompt]`;
    }
    ...
    // v1.10: per-phase resolution diagnostic line (only when something
    // other than a clean exact match happened).
    if (!verbose) {
      const resLine = _formatResolutionLine(resolvedSkill, phase.skill);
    ...
    for (const pl of promptLines) {
      ...
      // Attestation contract: emitted only for non-verbose phases with a routed skill.
      if (!verbose && resolvedSkill.source !== 'absent') {
        lines.push('');
        lines.push(`[attestation] If you load the routed skill via your harness's skill tool, end`);
        lines.push(`your mark-complete summary with a line of the form:`);
        lines.push(`    invoked_skill: ${skillLine}`);
        lines.push(`If you fell back to inline execution because the skill was unavailable,`);
        lines.push(`report:`);
        lines.push(`    invoked_skill: (inline-fallback)`);
      }
...
function startRun(templateNameOrPath, opts) {
  ...
  if (binding === 'quick') {
    ...
    // Filter out child phases (those with `parent:` set) — they're
    // materialized at runtime when the parent fans out. Scaffolding
    // them as top-level dirs causes bleed-through pollution.
    const topLevelPhases = tpl.phases.filter(function (p) {
      return !Object.prototype.hasOwnProperty.call(p, 'parent');
    });
    const flatOrder = flatTopoSort(topLevelPhases);
    const phaseNumByPhaseId = {};
    let phaseCounter = nextPhaseNum(projectDir);
    for (const phase of flatOrder) {
      // P3 (Bug G): skip workflow-meta phases — they must not appear as ROADMAP
      // entries. Two categories:
      //   kind:scaffold  — plumbing commands (e.g. setup health-checks), not
      //                    real deliverables.
      //   materialize:roadmap-phases parent (parent == null) — fan-out
      //                    controller; the actual ROADMAP entries are the
      //                    per-item children injected by materializeRoadmapPhases()
      //                    at mark-complete time.
      // We intentionally do NOT record these ids in phaseNumByPhaseId. A missing
      // entry resolves to undefined, which satisfies the `phaseNum != null` check
      // in markPhaseComplete and silently skips the per-dir SUMMARY write —
      // the desired no-op for plumbing phases (see P3 redirect below).
      const isMetaPhase = phase.kind === 'scaffold'
        || (phase.materialize === MATERIALIZE_ROADMAP_PHASES && phase.parent == null);
      if (isMetaPhase) continue;
...
function resumeRun(slug, opts) {
  ...
  workflow.applyAutoInjectFinalize(tpl);
  const waves = workflow.computeWavesWithFanout(tpl, state.parent_outputs || {});
...
/**
 * Parse the `invoked_skill:` attestation line from a mark-complete stdin payload.
 * Returns the trimmed skill name, or `'<unrecorded>'` if the line is absent.
 *
 * @param {string} text
 * @returns {string}
 */
function _parseInvokedSkill(text) {
  if (typeof text !== 'string') return '<unrecorded>';
  const m = text.match(/^invoked_skill:\s*(.+?)$/m);
  return m ? m[1].trim() : '<unrecorded>';
}

/**
 * Persist the `invoked_skill` attestation for a phase run to
 * `.planning/.run-state/<slug>/<phaseId>.json`.
 * Merges into an existing file if one already exists; never throws.
 *
 * @param {string} slug
 * @param {string} phaseId
 * @param {string} invokedSkill
 * @param {string} projectDir
 */
function _writeInvokedSkillState(slug, phaseId, invokedSkill, resolvedSkill, projectDir) {
  try {
    const stateDir = path.join(projectDir, '.planning', '.run-state', slug);
    const stateFile = path.join(stateDir, `${phaseId}.json`);
    let existing = {};
    try { existing = JSON.parse(fs.readFileSync(stateFile, 'utf8')); } catch (_) {}
    const updated = Object.assign({}, existing, { invoked_skill: invokedSkill, resolved_skill: resolvedSkill });
    lifecycle.writeFile(stateFile, JSON.stringify(updated, null, 2) + '\n');
  } catch (_) {
    // Never block phase completion due to attestation write failures.
  }
}
...
function markPhaseComplete(slug, phaseId, summaryText, opts) {
  ...
  const tpl = workflow.loadTemplate(state.template_path, {projectDir});
  workflow.applyAutoInjectFinalize(tpl);
  let waves = workflow.computeWavesWithFanout(tpl, state.parent_outputs || {});
  ...
  // Persist invoked_skill attestation to .planning/.run-state/<slug>/<phaseId>.json.
  // Also persist resolved_skill so the audit rule can skip scaffold/skill-less phases.
  // Errors are swallowed — attestation write must never block phase completion.
  const currentPhaseForAttestation = currentWave.find(function(p) { return p && p.id === phaseId; });
  const phaseSkillResult = resolvePhaseSkill(
    currentPhaseForAttestation && currentPhaseForAttestation.skill,
    {projectDir}
  );
  const resolvedSkillForState = phaseSkillResult.source === 'absent' ? '(absent)' : (phaseSkillResult.name || '(absent)');
  _writeInvokedSkillState(slug, phaseId, _parseInvokedSkill(summaryText), resolvedSkillForState, projectDir);
  ...
  if (binding === 'quick') {
    ...
    // milestone binding
    const currentPhase = currentWave.find(function (p) { return p && p.id === phaseId; });
    const phaseNumByPhaseId = Object.assign({}, state.phaseNumByPhaseId || {});

    // Resolve the phase dir for SUMMARY.md write. For expanded child rows
    // (e.g. 'child-plan::item-1'), fall back to the item.id mapping that
    // was established when the parent fanned out.
    let phaseNum = phaseNumByPhaseId[phaseId];
    if (phaseNum == null && currentPhase && currentPhase.item && typeof currentPhase.item.id === 'string') {
      phaseNum = phaseNumByPhaseId[currentPhase.item.id];
    }
    if (phaseNum != null) {
      ...
      // P3 (Bug G): plumbing phase (kind:scaffold etc.) — no ROADMAP phase dir
      // exists because startRun's meta-phase filter skipped it intentionally.
      // Preserve the summary by appending it to the milestone dir's SUMMARY.md
      // so output is never silently dropped.
      //
      // Exception: materialize:roadmap-phases parents have no phase dir either
      // (also filtered by P3), but their output is the structured fan-out JSON;
      // materializeRoadmapPhases() below consumes it and writes per-item PLAN.md
      // files, so we must NOT also write it here.
      !(currentPhase && currentPhase.materialize === MATERIALIZE_ROADMAP_PHASES && currentPhase.parent == null)
    ) {
      const msDir = path.dirname(milestoneRunPath(slug, projectDir));
      const msSummaryPath = path.join(msDir, 'SUMMARY.md');
      let existing = '';
      try { existing = fs.readFileSync(msSummaryPath, 'utf8'); } catch (_) {}
      lifecycle.writeFile(msSummaryPath,
        existing + `\n## Workflow phase: ${phaseId}\n\n${summaryText}\n`);
    }

    // Fan-out: when the just-completed phase is a `materialize: roadmap-phases`
    // parent, parse its structured-list output, scaffold one ROADMAP entry per
    // item, persist the parent output, and recompute waves so subsequent waves
    // dispatch the expanded child phases (one per item × child template).
    const parentOutputs = Object.assign({}, state.parent_outputs || {});
    let didMaterialize = false;
    if (
      currentPhase
      && currentPhase.materialize === MATERIALIZE_ROADMAP_PHASES
      && currentPhase.parent == null
      && !Object.prototype.hasOwnProperty.call(parentOutputs, phaseId)
    ) {
      const patch = materializeRoadmapPhases(
        currentPhase, summaryText, state, projectDir, { tpl }
      );
      Object.assign(parentOutputs, patch.parent_outputs_patch);
      Object.assign(phaseNumByPhaseId, patch.phaseNumByPhaseId_patch);
      didMaterialize = true;
    }
    ...
    // If we materialised, recompute waves so the next-wave instruction sees
    // the freshly-expanded child phases.
    if (didMaterialize) {
      waves = workflow.computeWavesWithFanout(tpl, parentOutputs);
    }
...
module.exports = {
  ...
  formatInstruction,
  resolvePhaseSkill,
  // Exposed for unit testing of the fan-out helper; not part of the public API.
  materializeRoadmapPhases,
  // Exposed for testing (Bug F)
  _parseInvokedSkill,
};
