 ...
 * Tries custom → phase → milestone in that order.
 * @param {string} slug
 * @param {object} [opts]
...
function resolveBinding(slug, opts) {
  ...
  try {
    ...
    if (state) return {binding: 'custom', state, projectDir};
  } catch (_) {}
  // 2. phase
...
function formatInstruction(template, wave, waveIndex, opts) {
  ...
  for (const phase of wave) {
    ...
    const skill = (phase.skill != null) ? phase.skill : '(absent)';
...
function startRun(templateNameOrPath, opts) {
  ...
  const binding = tpl.meta.binds_to || 'custom';
  ...
  if (o.dryRun) {
    ...
    const wavesWithInstructions = waves.map(function(wave, i) {
      return {
        phases: wave,
        instruction: formatInstruction(tpl, wave, i, {projectDir, slug: drySlug, totalWaves: waves.length}),
      };
    });
