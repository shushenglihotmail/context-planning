 ...
 *
 * Each `parentOutputs[parentId]` value may be either:
 *   - a bare items array (legacy / `optimizable: false`), or
 *   - a `{ optimizable, items }` object as returned by `parseParentOutput`.
 *
 * Cross-item ordering follows the `optimizable` flag
 * (see `runtime-fanout.resolveItemOrder`):
 *   - array mode (default, `optimizable: false` or missing): item N's child
 *     subtree waits for item N-1's child subtree. Any per-item `depends_on`
 *     is ignored.
 *   - dag mode (`optimizable: true`): item B's child subtree waits for
 *     every expanded child of every item it `depends_on`. Items without a
 *     `depends_on` field are treated as `depends_on: []` (parallel root).
 *
...
function expandPhases(phases, parentOutputs) {
  ...
  for (const phase of topLevel) {
    ...
    const rawOutput = outputs[parentId];
    const items = Array.isArray(rawOutput)
      ? rawOutput
      : (rawOutput && Array.isArray(rawOutput.items) ? rawOutput.items : undefined);
    const optimizable = !Array.isArray(rawOutput) && rawOutput && typeof rawOutput.optimizable === 'boolean'
      ? rawOutput.optimizable
      : false;

    if (!Array.isArray(items)) {
      throw new Error(`fanout: parent '${parentId}' output must be an array or {items, optimizable} object`);
    }
    ...
    const mode = resolveItemOrder({ optimizable, items });
