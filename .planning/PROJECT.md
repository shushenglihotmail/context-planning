...
- ✓ Template parameterization whitelist: restrict workflow + phase template parameterization to whitelisted content-bearing fields (`skill`, `prompt`, `description`, `max_children`, `min_children`); hard-ban `{{item.X}}` tokens and unresolved `{{...}}` after expansion. Closes the silent-substitution bug from the v1.6 docs workflow. — v1.7
- ✓ Milestone workflow end-to-end + cp quick attach-by-name: `milestone.yaml` fan-out fix (child-plan + child-execute under `propose-phases` mirroring quick.yaml), workflow validator hard-fail on `materialize: roadmap-phases` parents missing `supervised: true` or `parent:` child template, `cp quick --project <name>` / `--milestone <name>` name-based attach backed by `~/.config/cp/projects.json`, `cp project list|rm` + `cp milestone list` commands, `--project-path` deprecated. — v1.8 (published as `context-planning@1.8.0`)
- ✓ Implicit-attach for `cp quick` when `--project` omitted: cwd-based project resolution, sticky default when single registered project, friendly error when ambiguous. — v1.8.1 (published as `context-planning@1.8.1`)
- ✓ Bug G — milestone fan-out wired: `materialize: roadmap-phases` parent prompts now augmented with the structured-list JSON contract (`buildParentPrompt`); `markPhaseComplete` for milestone runs parses parent output, enforces min/max children, scaffolds one ROADMAP phase per item with `PLAN.md`, persists parent output, and injects expanded child waves via new `workflow.computeWavesWithFanout`. Workflow plumbing phases (`kind: scaffold`, fan-out parents) suppressed from initial ROADMAP scaffold; their summaries redirect to milestone-dir `SUMMARY.md`. Shared `kahnWaves` helper extracted (3 sites). End-to-end integration test added (`test/integration-fanout-milestone-v18.js`). — v1.8.2
- ✓ Framework Bug Fixes from v1.8.1 Retro: six bugs (A `cp status` h4 blindness, B `writeSummary` phantom paths, C `complete-milestone` stale STATE banner, D `doctor --fix-dual-plan` autofix, E `sham-review-log` MEDIUM audit rule, F skill-load attestation via `[attestation] invoked_skill:` contract + `.planning/.run-state/<slug>/<phase>.json` persistence + `skill-resolved-but-not-loaded` MEDIUM audit rule) plus cross-cutting fix `f2ca64a` for false-positive attestation findings on scaffold phases and a misleading deprecation warning. 9 commits across phases 105–110. Milestone driven through `cp run milestone` to dogfood v1.8.2's Bug G fix. — v1.9 (published as `context-planning@1.9.0`)
...
### Active
- **v1.10 Skill Routing & Verify Gate**: fix two real framework bugs from a downstream consumer's v4.7 retro. (1) `lib/provider.js#resolveSkill` checks provider install but not per-skill `SKILL.md` existence — add per-skill stat + parens-sigil opt-in fuzzy matching (`(skill-name)`) with role-filtered token-overlap algorithm + 5-step resolution chain (sigil-fuzzy → exact → manual prompt inline → subagent-dispatch directive → bare prompt) + ONE consolidated workflow-start diagnostic. (2) `templates/workflows/milestone.yaml` review phase is prose-only — add new `cp run-verify` scaffold subcommand reading `behavior.test_command` (auto-detect chain: `npm test` → `pytest` → `cargo test` → `go test ./...`) with new `verify` phase inserted before `review` via `depends_on`, opt-out via `behavior.verify: false` / `--skip-verify`. Plus README PowerShell-on-Windows blurb (`cp` aliases to `Copy-Item`; use `cp.cmd`). 5 phases. Spec: `docs/superpowers/specs/2026-06-02-v1.10-skill-routing-verify-gate-design.md`. — v1.10
...
| Parens-sigil opt-in fuzzy matching `(skill-name)` | Parens 100% YAML-safe at any position; sigil is a flag ("fuzzy allowed") not a glob pattern; in-place sigil scales naturally where whole-property `X_skill_fuzzy: true` doesn't | v1.10 P1 — `lib/provider.js` parser + role-filtered token-overlap matcher |
| Verify gate is `kind: scaffold` (real CLI exec) | LLM-in-the-loop review can be fake-attested past; only objective CLI execution provides a real verification boundary | v1.10 P3/P4 — new `cp run-verify` + `verify` phase before `review` |
| Sigil opt-in policy: strict-by-default, fuzzy-only-when-no-SP-match | When SP ships an exact-match skill, deterministic canonical invocation > fuzzy; reserve fuzzy for genuine catalog gaps (e.g., no `code-review` skill exists in SP) | v1.10 P5 docs + concrete `milestone.yaml`: `brainstorm_skill: brainstorming` strict, `review_skill: (code-review)` fuzzy |

---
*Last updated: 2026-06-02 — opened v1.10 Skill Routing & Verify Gate milestone (5 phases): per-skill availability check with parens-sigil opt-in fuzzy matching + new `cp run-verify` scaffold subcommand inserted before review phase. Fixes 2 real bugs from a downstream consumer's v4.7 retro; 3 retro items already fixed in v1.9 or were non-bugs.*
