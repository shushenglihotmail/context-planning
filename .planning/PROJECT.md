...
- **Milestone workflow end-to-end + cp quick attach-by-name**: (A) Fix `milestone.yaml`'s `roadmap-phases` fan-out gap by adding `child-plan` + `child-execute` phases under `propose-phases` (mirrors `quick.yaml` shape), plus `execute_skill` / `execute_role` params. (B) Harden the workflow validator: hard-fail any `materialize: roadmap-phases` parent missing `supervised: true` or a `parent: <id>` child template; verify supervisor dispatch + ROADMAP status write-back end-to-end. (C) Add `cp quick --project <name>` / `--milestone <name>` name-based attach backed by a `~/.config/cp/projects.json` registry; new `cp project list`/`rm` and `cp milestone list` commands; deprecate `--project-path` with runtime warning. Spec: `.planning/milestones/milestone-workflow-end-to-end-quick-attach/DESIGN.md`. — v1.8
...
---
*Last updated: 2026-05-29 — started v1.7 Template parameterization whitelist milestone*
